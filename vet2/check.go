		...
		for i, stmt := range v.decl.Body.List {
			...
			ast.Inspect(stmt, func(node ast.Node) bool {
				...
				case *ast.AssignStmt:
					...
					checkStmt(assign, lhs, rhs, lhsFlag, rhsFlag, skipFlag, fset)
				case *ast.DeclStmt:
				...
				case *ast.RangeStmt:
					...
					rhsFlag := collectRhsFlag(rhs, skipFlag)
					rhsFlag |= rhsFlag << 1
					checkStmt(false, lhs, rhs, lhsFlag, rhsFlag, skipFlag, fset)
				}
				return true
			})
		}
...
func checkStmt(isAssign bool, lhs, rhs []ast.Expr, lhsFlag, rhsFlag uint64, skipFlag uint64, fset *token.FileSet) {
