	...
	case *ast.ExprStmt:
		...
		unwrapExpr(e.X, func(expr ast.Expr) {
			if getExprReadonlyFlag(expr) > 0 {
				...
				pass.Reportf(expr.Pos(), "cannot be assigned")
			}
		})
	case *ast.IncDecStmt:
		if getExprReadonlyFlag(e.X) > 0 {
			...
			log.Printf("[lhs:%s,at %v ] cannot be assigned\n", buf.String(), fset.Position(e.X.Pos()))

			pass.Reportf(e.X.Pos(), "cannot be assigned")
		}
	case *ast.AssignStmt:
	...
	if isAssign {
		for i := 0; i < len(lhs); i++ {
			...
			if lhsFlag&(1<<i) != 0 {
				...
				pass.Reportf(lhs[i].Pos(), "cannot be assigned")
			}
		}
	}
...
`,
			...
			rhsBuf.String(), fset.Position(rhsPos))

		pass.Reportf(lhs[i].Pos(), `variable [lhs:%s at %v ] cannot assigned with 
					variable readonly [rhs:%s at %v ]
`,
			lhsBuf.String(), fset.Position(lhs[i].Pos()),
			rhsBuf.String(), fset.Position(rhsPos))
